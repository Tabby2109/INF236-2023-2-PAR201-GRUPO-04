...
function getElementRight() {
    const popupWidth = (element && (element.classList.contains('e-date-wrapper') || element.classList.contains('e-datetime-wrapper') || element.classList.contains('e-date-range-wrapper') || element.classList.contains('e-ddl') || element.classList.contains('e-multiselect'))) ? (popupRect ? popupRect.width : 0) :
        ...
        for (const prop of Object.keys(newProp)) {
            switch (prop) {
                ...
                case 'position':
                    this.checkPositionData();
                    if (this.isModal) {
                        const positionX = !isNullOrUndefined(oldProp.position) && !isNullOrUndefined(oldProp.position.X) ? oldProp.position.X : this.position.X;
                        const positionY = !isNullOrUndefined(oldProp.position) && !isNullOrUndefined(oldProp.position.Y) ? oldProp.position.Y : this.position.Y;
...
let Tooltip = class Tooltip extends Component {
    ...
    calculateTooltipOffset(position, xScalingFactor = 1, yScalingFactor = 1) {
        ...
        let tipWidth, tipHeight, tooltipEleWidth, tooltipEleHeight, arrowEle;
        let tipAdjust, tipHeightAdjust, tipWidthAdjust;
    ...
    addDataTooltipId(target, id) {
        attributes(target, { 'data-tooltip-id': id });
    }
    removeDataTooltipId(target) {
    ...
    collisionFlipFit(target, x, y) {
        ...
        if (this.tooltipEventArgs.cancel) {
            ...
            let elePosVertical = elePos.vertical;
            let elePosHorizontal = elePos.horizontal;
