...
let Slider = class Slider extends Component {
        ...
        if (this.isMaterialTooltip) {
            ...
            if (this.type === 'Default') {
                setTimeout(() => { if (this.tooltipElement)
                    this.tooltipElement.style.transition = this.transition.handle; }, 2500);
            }
            else {
                setTimeout(() => { if (this.tooltipElement)
                    this.tooltipElement.style.transition = 'none'; }, 2500);
            }
        }
    }
    ...
    sliderBarClick(evt) {
        ...
        let transition = (this.isMaterial || this.isMaterial3) && this.tooltip.isVisible ?
...
let FormValidator = FormValidator_1 = class FormValidator extends Base {
    ...
    getErrorMessage(ruleValue, rule) {
        ...
        if (!isNullOrUndefined(formats)) {
            for (let i = 0; i < formats.length; i++) {
                const value = ruleValue instanceof Array ? ruleValue[parseInt(i.toString())] : ruleValue;
    ...
    removeErrorRules(name) {
        for (let i = 0; i < this.errorRules.length; i++) {
            const rule = this.errorRules[parseInt(i.toString())];
            if (rule.name === name) {
    ...
    renderSelectedFiles(args, targetFiles, directory, paste) {
        ...
        if (this.enableHtmlSanitizer) {
            for (let i = 0; i < fileData.length; i++) {
                ...
                const currentFileName = fileData[parseInt(i.toString())].name;
                let isUTF8 = false;
                ...
                let hasHTMLString = htmlTagRegex.test(currentFileName);
                if ((sanitizeFileName !== fileData[parseInt(i.toString())].name) && !(isUTF8 && !hasHTMLString)) {
                    const encodedFileName = targetFiles[parseInt(i.toString())].name.replace(/[\u00A0-\u9999<>\&]/g, function (i) {
