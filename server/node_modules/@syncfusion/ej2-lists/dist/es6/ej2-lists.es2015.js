        ...
        if (this.showCheckBox && this.isValidLI(args.item)) {
            ...
            if (typeof this.dataSource[0] !== 'string' && typeof this.dataSource[0] !== 'number') {
                ...
                if (this.enablePersistence && !isNullOrUndefined(this.selectedId)) {
                    let index = this.selectedId.findIndex(e => e == fieldData[this.listBaseOption.fields.id].toString());
                    if (index != -1) {
...
class Virtualization {
    ...
    createUIItem(args) {
        ...
        if (this.listViewInstance.showCheckBox) {
            ...
            if ((!isNullOrUndefined(this.listViewInstance.virtualCheckBox)) &&
                ...
                const nodes = this.templateFunction(this.templateData, this.listViewInstance);
        ...
        if (this.listViewInstance.fields.groupBy) {
            const nodes = this.groupTemplateFunction(this.headerData, this.listViewInstance);
            if (this.listViewInstance.template && this.listViewInstance.isReact) {
                this.listViewInstance.renderReactTemplates();
            }
            [].slice.call(nodes).forEach((ele) => {
        ...
        for (let i = 0; i < groupTemplate.children.length; i++) {
    ...
    compileTemplate(element, item, isHeader) {
        ...
        subNode.onChange = (value) => {
            ...
            let currentTemplate = templateElements;
            if (value.isHeader) {
                currentTemplate = groupTemplateElements;
            }
            [].slice.call(currentTemplate).forEach(function (currentElement) {
                let RemovableClass = element.classList.value.replace('e-checkbox', '');
                ...
                for (let i = 0; i < element.attributes.length; i++) {
                    if (element.attributes[i].name != 'class') {
                        const newData = currentElement.getAttribute(element.attributes[i].name);
    ...
    updateUI(element, index, targetElement) {
        // eslint-disable-next-line @typescript-eslint/ban-types
        const onChange = this.isNgTemplate() ? this.onNgChange : (this.isVueFunctionTemplate()) ? this.onVueChange : this.onChange;
     ...
     */
    onVueChange(newData, listElement, virtualThis) {
        let liItem = ListBase.createListItemFromJson(virtualThis.listViewInstance.createElement, 
        // eslint-disable-next-line
...
let Sortable = Sortable_1 = class Sortable extends Base {
    ...
    keyDownHandler(e) {
        if (e.keyCode === 27) {
            let dragStop = getComponent(this.element, 'draggable');
            if (dragStop) {
